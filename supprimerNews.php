<?php
    ...
    $newsList = $bdd->query("SELECT id_news, titre FROM news ORDER BY date_publication DESC")->fetchAll(PDO::FETCH_ASSOC);

    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        // Suppression de la news
        if (isset($_POST['id_news'])) {
            $requete = $bdd->prepare("DELETE FROM news WHERE id_news = :id_news");
